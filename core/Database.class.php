<?Php
    class Database{
        ...
        public function getRows($table, $values = array(), $offset= false, $order = false, $limit = 1000){
            $prep = array();
            $data = array();
            foreach($values as $k => $v){
                $prep[] = "$k = ?";
            ...
            $data = array_values($values);
            $prep = implode(' AND ', $prep);
            $query = $this->db->prepare("SELECT * FROM $table ".($values ? " WHERE  $prep" : ""). ($order ? " ORDER BY $order" : "")." LIMIT $limit");
            $query->execute($data);
